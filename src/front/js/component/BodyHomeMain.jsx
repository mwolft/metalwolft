import React from "react";

export const BodyHomeMain = () => {

    return (
        <div className="container my-5">
            <div className="row d-flex flex-column">
                <div className="mt-2 mx-2 col-12 col-sm-12 col-md-10 col-lg-7 col-xl-6">
                    <h1 className="h1-home">CARPINTERÍA METÁLICA EN <span className="span-home">CIUDAD REAL.</span> <br />
                    <span style={{fontSize: '20px', fontStyle: 'italic'}}>Tu herrero soldador de confianza.</span></h1>
                </div>
                <div className="mt-2 mx-3 col-3 col-sm-3 col-md-2 col-lg-1 col-xl-1">
                    <hr className="hr-home"></hr>
                </div>
            </div>
        </div>
    );
};

