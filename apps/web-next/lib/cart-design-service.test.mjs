...
assert.match(cartView, /Diseño previo de esta reja en \{designPreviewDimensions\}/);
...
assert.match(styles, /\.mw-cart-design-preview\s*{[^}]*grid-template-columns:\s*2\.1rem minmax\(0, 1fr\);/s);
assert.match(styles, /\.mw-cart-design-preview\s*{[^}]*border-top:\s*1px solid/);
assert.doesNotMatch(styles.match(/\.mw-cart-design-preview\s*{[^}]*}/)?.[0] ?? "", /border-radius|background:/);
assert.match(styles, /\.mw-cart-design-preview__heading\s*{[^}]*justify-content:\s*space-between;/s);
