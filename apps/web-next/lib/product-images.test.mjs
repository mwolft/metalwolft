...
assert.match(gallerySource, /horizontalSources\.has\(selectedImage\.src\)/);
assert.match(gallerySource, /style=\{horizontalSources\.has\(selectedImage\.src\) \? \{ objectFit: "cover" \} : undefined\}/);
assert.match(gallerySource, /width <= height/);
assert.match(gallerySource, /event\.currentTarget\.naturalWidth/);
assert.match(gallerySource, /event\.currentTarget\.naturalHeight/);
assert.doesNotMatch(gallerySource, /style=\{selectedRatio|imageRatios|rememberImageRatio/);
